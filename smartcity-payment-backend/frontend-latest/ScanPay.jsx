...
// baseFare: 이용 시 무조건 부과되는 최소 기본 요금 (백엔드 fareEngine 동일)
const BASE_FARE_USDC = 0.01;
const SERVICE_TYPES = [
  { id: "parking",     label: "주차",        emoji: "🚗", serviceId: "PARKING-LOT-A-001", rate: "0.02 USDC/min", baseFare: BASE_FARE_USDC, depositUsdc: 5.0 },
  { id: "bicycle",     label: "공유 자전거", emoji: "🚴", serviceId: "BIKE-STATION-007",  rate: "0.01 USDC/min", baseFare: BASE_FARE_USDC, depositUsdc: 3.0 },
  { id: "ev_charging", label: "전기차 충전", emoji: "⚡", serviceId: "EV-CHARGER-B-003",  rate: "0.25 USDC/kWh", baseFare: BASE_FARE_USDC, depositUsdc: 10.0 },
];
      ...
      <div className="max-w-md mx-auto px-4 pt-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
          ...
          <p className="text-xs text-muted-foreground mt-3">
            예치금 {selectedService?.depositUsdc} USDC
          </p>
          {fareInfo ? (
            <div className="mt-3 space-y-1">
              <p className="text-xl font-bold text-primary">{fareInfo.fareUsdc} USDC</p>
              <p className="text-[10px] text-muted-foreground">
                기본 {BASE_FARE_USDC} USDC + 이용 {Math.max(0, parseFloat(fareInfo.fareUsdc || 0) - BASE_FARE_USDC).toFixed(6)} USDC
              </p>
            </div>
          ) : (
            <div className="mt-3 space-y-1">
              <p className="text-sm text-muted-foreground">최소 요금 {BASE_FARE_USDC} USDC 부과</p>
              <p className="text-[10px] text-muted-foreground/60">이용 시간에 따라 추가 청구</p>
            </div>
          )}
        </motion.div>
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col items-center justify-center px-6 pb-24">
      ...
      <p className="text-sm text-muted-foreground mb-6 text-center">
        {selectedService?.label} · {fmt(elapsed)} 이용
      </p>
      <div className="w-full max-w-sm rounded-2xl bg-card border border-border p-5 mb-8 space-y-3 text-sm font-mono">
        {[
          ...
          ["이용 시간", fmt(elapsed)],
          ["기본 요금", `${BASE_FARE_USDC} USDC (최소 요금)`],
          ["이용 요금", fareInfo?.fareUsdc ? `${Math.max(0, parseFloat(fareInfo.fareUsdc) - BASE_FARE_USDC).toFixed(6)} USDC` : "-"],
          ["최종 요금", fareInfo?.fareUsdc ? `${fareInfo.fareUsdc} USDC` : "-"],
          ["체인", "Base Sepolia"],
          ["정산", "온체인 에스크로"],
        ].map(([k, v]) => (
